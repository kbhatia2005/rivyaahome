...
function setupHero(){
  const meta = CAT_META[currentCat] || {label:currentCat, desc:'Handcrafted with love.', color:'linear-gradient(135deg,#5a3020,#8b5e3c)'};
  ...
  if(hero) hero.style.background = meta.color;
}
...
function fetchProductsJSONP(){
  ...
  const cbName = 'catCb_' + Date.now();
  const script = document.createElement('script');
  const timeout = setTimeout(()=>{
    if(loading) loading.innerHTML = '<i class="fa-solid fa-triangle-exclamation" style="font-size:36px;opacity:.4;display:block;margin-bottom:10px"></i><p>Could not load. Check internet.</p>';
  ...
  script.onerror = ()=>{ clearTimeout(timeout); if(loading) loading.style.display='none'; if(empty) empty.style.display='block'; };
...
function renderProducts(){
  ...
  filtered.forEach((p,idx)=>{
    ...
    card.style.cssText='opacity:0;transform:translateY(24px);transition:opacity .4s ease,transform .4s ease';
    ...
    card.innerHTML=`
      ...
      <div class="prod-body">
        <div class="prod-cat-tag">${CAT_META[p.category]?.label||p.category}</div>
        ...
        <div class="prod-btns">
          ...
          <div class="btn-row">
            <button class="btn-buy" onclick="buyNow('${safeName}','${p.price}')">
    ...
    setTimeout(()=>{ card.style.opacity='1'; card.style.transform='translateY(0)'; }, 60+idx*90);
  });
}
...
function removeFromCart(id){saveCart(getCart().filter(i=>i.id!==id));updateCartBadge();renderCartItems();}
...
function updateCartBadge(){
  ...
  const b=document.getElementById('cartBadge');if(b)b.textContent=t;
}
...
function renderCartItems(){
  ...
  const totalEl=document.getElementById('cartTotal');if(totalEl)totalEl.textContent='₹'+total.toLocaleString('en-IN');
  if(waBtn){
